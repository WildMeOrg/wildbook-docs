...
Match candidates with similar viewpoints are displayed in match results (ex: left compared with front left or back left). If the annotation viewpoint is incorrect (ex: the animal's left side saved as its right side), you can delete your annotation and [manually create a new one](../data/manual-annotation-beta.md#creating-a-manual-annotation). You won't be able to edit the viewpoint of someone else's image unless you have an edit [Collaboration](../security/silo-security/index.md#collaborations) with that user.
